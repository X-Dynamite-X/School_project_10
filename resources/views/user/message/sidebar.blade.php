<div class="w-1/5 bg-white border-r overflow-y-auto fexd my_h_93 min-w-80">
  ...
  {{-- @if ($conversation->user1_id == auth()->user()->id)
                            ...
                            @else
                                ...
                                <span class="block">
                                    @if ($conversation->user2->last_seen_at)
                                        Last seen
                                        {{ \Carbon\Carbon::parse($conversation->user2->last_seen_at)->diffForHumans(['short' => true]) }}
                                    @else
                                        No data available
                                    @endif
                                </span>
            ...
            @elseif ($conversation->user2_id == auth()->user()->id)
                <li id="user_{{ $conversation->user2_id }}" class="showConversation flex justify-between gap-x-6 py-5"
                    data-conversation_id="{{ $conversation->id }}">
                    <div class="flex min-w-0 gap-x-4">
                        <img class="h-12 w-12 flex-none rounded-full bg-gray-50"
                    ...
                    <div class="hidden shrink-0 sm:flex sm:flex-col sm:items-end">
                        <p class="text-sm leading-6 text-gray-900">Co-Founder / CEO</p>
                        <p class="text-xs status">
                            @if (Helpers::isUserOnline($conversation->user1->id) == 'Online')
                                <span class="text-green-500 inline">
                                    {{ Helpers::isUserOnline($conversation->user1->id) }}
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor"
                                        class="size-4 inline">
                                        <path fill-rule="evenodd"
                                            d="M8 15A7 7 0 1 0 8 1a7 7 0 0 0 0 14Zm3.844-8.791a.75.75 0 0 0-1.188-.918l-3.7 4.79-1.649-1.833a.75.75 0 1 0-1.114 1.004l2.25 2.5a.75.75 0 0 0 1.15-.043l4.25-5.5Z"
